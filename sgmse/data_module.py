...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def __init__(
        self, base_dir, clean_dir, noisy_dir, loss_type, dataset="tcd", format='default', batch_size=8,
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:  
            self.train_set = Specs(data_dir=self.base_dir,dataset= self.dataset,clean_dir=self.clean_dir, noisy_dir=self.noisy_dir, subset='train',
                dummy=self.dummy,supervised=self.supervised, loss_type =self.loss_type ,shuffle_spec=True, format=self.format,
            ...
            self.valid_set = Specs(data_dir=self.base_dir,dataset= self.dataset,clean_dir=self.clean_dir, noisy_dir=self.noisy_dir, subset='valid',
                dummy=self.dummy,supervised=self.supervised, loss_type =self.loss_type ,shuffle_spec=False, format=self.format,
                normalize=self.normalize, **specs_kwargs)
